...
> You are a creator with 18,000 Instagram followers in Mexico City.
> Brands want to reach your audience — but you have no bank account,
> no payment processor, and no way to prove your reach.
> DigiPaga AI solves all three in one step.
> Connect your profile → your Commerce Score proves your value →
> the agent matches you to campaigns → someone buys →
> **USDC lands in your wallet before you finish reading this sentence.**

What makes it different:

1. **The wallet appears automatically** — Privy creates an embedded MPC wallet the moment you sign in. No seed phrase. No "create wallet" step.
2. **Your handle IS your payment address** — `@aliciaq` on Instagram maps directly to your Circle Agent Wallet on Arc. Send to a handle, not a hex address.
3. **The agent acts, not just recommends** — DigiAgent executes swaps, moves funds to yield, and settles payouts autonomously within user-defined policy limits.
...
digi-agent-ai/
├── apps/
│ ├── landing/ ← marketing site — no auth, pure storytelling
│ ├── web/ ← the app — 10 screens, wallet, DigiAgent chat
│ └── api/ ← Express 5 — auth, wallet, social, agent routes
├── packages/
...
**Frontend** — Next.js 16 · React 19 · TypeScript · TailwindCSS · shadcn/ui · Framer Motion · Privy

**Backend** — Express 5 · Prisma · PostgreSQL (Neon) · Redis (Upstash) · modular services

**Money layer** — Arc testnet · Circle Agent Wallets · Circle Nanopayments · Circle App Kits · USDC · EURC

**AI** — Qwen3-Instruct · Llama 3.3 70B · Kimi K2 · DeepSeek V3 (via OpenRouter)

**Infra** — Turborepo · pnpm · Vercel (frontend) · Railway (API)
...
Other agents on Arc can read this file to discover what DigiPaga AI can do autonomously, its policy limits, spend caps, and how to send structured payout commands. Future: MCP endpoint for agent-to-agent tool calls.
...
- [x] Circle SDK wrapper — Agent Wallets, Nanopayments
- [x] Arc testnet RPC connected
- [x] Agent capability manifest
- [x] `apps/landing` — marketing site folder
- [ ] Privy auth — embedded wallet on Google / X / Telegram / SMS login
- [ ] Real Commerce Score from connected social accounts
- [ ] Live USDC payout on Arc testnet via Circle Agent Wallet
- [ ] Fan nanopayments — gas-free tips via Circle Gateway
- [ ] Voice commands — speak to pay, swap, and earn
- [ ] Agent-to-agent marketplace (v2)
...
**Programmable Money Hackathon** · Encode Club × Arc × Circle  
**Track: Agentic Economy** — autonomous agents that hold wallets, pay, settle jobs, and transact with other agents using USDC.
...
<div align="center">
  <sub>Built on <a href="https://arc.io">Arc</a> · Powered by <a href="https://circle.com/usdc">USDC</a> · Auth by <a href="https://privy.io">Privy</a></sub>
</div>
